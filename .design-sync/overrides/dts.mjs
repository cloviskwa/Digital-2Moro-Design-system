...
function projectFor(pkgDir, typesRoot) {
  ...
  // forked from design-sync lib/dts.mjs — digital2moro-platform's source
  // uses `@/*` (-> src/*) path-alias imports throughout; tsc's declaration
  // emit does NOT rewrite those to relative specifiers, so the emitted .d.ts
  // tree still says `from '@/components/icons'` etc. Our tsc build mirrors
  // src/ 1:1 under PKG_DIR (rootDir: src, outDir: PKG_DIR), so `@/*` maps to
  // `<pkgDir>/*` in THIS build's own output — adding that mapping here lets
  // ts-morph's checker resolve those cross-file type imports instead of
  // treating them as unresolved modules.
